      ...
      for (int materialID : materialIDs) {
        ...
        for (int faceID=0;faceID<shape.mesh.material_ids.size();faceID++) {
          ...
          mesh->diffuseTextureID = loadTexture(model,
                                               ...
                                               modelDir);
          mesh->metalNess = (const float)materials[materialID].metallic;
          mesh->metalNessID = loadTexture(model,
              knownTextures,
              materials[materialID].metallic_texname,
              modelDir);

          mesh->roughNess= (const float)materials[materialID].roughness;
          mesh->roughNessID = loadTexture(model,
              knownTextures,
              materials[materialID].roughness_texname,
              modelDir);

          mesh->emissioin = materials[materialID].emission[0];
        }
