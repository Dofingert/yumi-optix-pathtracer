...
对于本文件的修改主要在SBT的构建部分，增加了自定义材质的支持内容。
  ...
  void SampleRenderer::buildSBT()
  {
    ...
    for (int meshID=0;meshID<numObjects;meshID++) {
      for (int rayID=0;rayID<RAY_TYPE_COUNT;rayID++) {
        ...
        rec.data.metalNess = mesh->metalNess;
        rec.data.emissioin = mesh->emissioin;
        rec.data.roughNess = mesh->roughNess;

        if (mesh->diffuseTextureID >= 0 && mesh->diffuseTextureID < textureObjects.size()) {
          rec.data.hasTexture[0] = true;
          rec.data.texture[0]    = textureObjects[mesh->diffuseTextureID];
        } else {
          rec.data.hasTexture[0] = false;
        }

        if (mesh->metalNessID >= 0 && mesh->metalNessID < textureObjects.size()) {
            rec.data.hasTexture[1] = true;
            rec.data.texture[1] = textureObjects[mesh->metalNessID];
        }
        else {
            rec.data.hasTexture[1] = false;
        }

        if (mesh->roughNessID >= 0 && mesh->roughNessID < textureObjects.size()) {
            rec.data.hasTexture[2] = true;
            rec.data.texture[2] = textureObjects[mesh->roughNessID];
        }
        else {
            rec.data.hasTexture[2] = false;
        }
